...
def postAtweet(filename,fileobj,token,oauth_consumer_secret, oauth_token_secret,status,in_reply_to_status_id,oauth_consumer_key,oauth_token):
...
# using tweepy to post a tweet 
    ...
    res = api.media_upload(filename,file=fileobj)
    print(res.media_id)
# end of tweepy    
...
#    GET https://api.twitter.com/1.1/statuses/mentions_timeline.json?count=2&since_id=14927799
    ...
    for mention in response:   
        ...
        fileobj = createImage(text['data'][0]['text'],mention['in_reply_to_screen_name'],url,'backgroundImage')               
        status = '@'+mention['user']['screen_name'] +' '+'@'+mention['in_reply_to_screen_name']
       
        postAtweet(mention['id_str'],fileobj,auto,oauth_consumer_secrete,oauth_secret_token,status,mention['id_str'],oauth_consumer_KEY,oauth_token)
